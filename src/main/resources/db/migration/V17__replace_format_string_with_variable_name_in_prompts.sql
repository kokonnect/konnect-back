...
(
    ...
    PRIMARY KEY (id),
    CONSTRAINT uk_prompt_slot_key UNIQUE (prompt_id, slot_key),
    CONSTRAINT uk_prompt_slot_order UNIQUE (prompt_id, slot_order)
);

-- 외래 키 제약 조건 설정
ALTER TABLE prompt_slot
    ADD CONSTRAINT FK_prompt_slot_template
        FOREIGN KEY (prompt_id)
            REFERENCES prompt_template (id);

UPDATE prompt_template
SET template = '다음 학교 가정통신문 텍스트를 분석하여 문서 유형을 분류해주세요.
...
WHERE module_name = 'CLASSIFICATION'
  AND id = 2;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('text', 1, 2);

UPDATE prompt_template
SET template = '다음 번역된 학교 가정통신문을 {{target_language}}로 요약해주세요.

## 요약 지침
- 핵심 내용을 3-5줄로 요약
- 중요한 날짜, 장소, 준비물 정보는 반드시 포함
- 학부모가 바로 이해할 수 있게 명확하게 작성
- {{target_language}}로 요약문만 출력하고 다른 설명은 하지 마세요
...
## 요약:'
WHERE module_name = 'SUMMARIZATION'
  AND id = 3;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('target_language', 1, 3),
       ('text', 2, 3);

UPDATE prompt_template
SET template = '다음 한국어 학교 가정통신문을 {{target_language}}로 번역해주세요.
...
## 번역:'
WHERE module_name = 'TRANSLATION' AND id = 4;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('target_language', 1, 4),
       ('text', 2, 4);

UPDATE prompt_template
SET template = '다음 한국어 학교 가정통신문을 외국인도 쉽게 이해할 수 있는 쉬운 한국어로 다시 작성해주세요.
...
## 쉬운 한국어 버전:'
WHERE module_name = 'SIMPLIFICATION' AND id = 5;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('text', 1, 5);

UPDATE prompt_template
SET template = '다음 한국어 학교 가정통신문에서 외국인이 이해하기 어려울 수 있는 한국어 표현들을 추출하고, 각 표현에 대해 {{target_language}}로 쉽게 설명해주세요.
...
- explanation: {{target_language}}로 쉽게 풀어서 설명 (간단명료하게, 순수 텍스트만)
- 최대 10개까지만 추출
- 정말 어려운 표현만 선별
...
WHERE module_name = 'DIFFICULT_EXPRESSION_EXTRACTION' AND id = 6;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('target_language', 1, 6),
       ('text', 2, 6);

UPDATE prompt_template
SET template = '다음 학교 가정통신문에서 모든 관련 정보를 추출해주세요.
...
## 중요: 출력 언어
- 모든 텍스트 필드는 반드시 {{target_language}}로 작성해주세요
- 원본이 한국어여도 {{target_language}}로 번역하여 출력
...
WHERE module_name = 'UNIFIED_EXTRACTION' AND id = 7;

INSERT INTO prompt_slot(slot_key, slot_order, prompt_id)
VALUES ('target_language', 1, 7),
       ('today', 2, 7),
       ('text', 3, 7);